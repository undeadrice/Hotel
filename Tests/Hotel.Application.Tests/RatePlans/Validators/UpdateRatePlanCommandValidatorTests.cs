...
public class UpdateRatePlanCommandValidatorTests
{
    private readonly UpdateRatePlanCommandValidator _validator = new();

    private static UpdateRatePlanCommand CreateValidCommand() =>
        new(
            Guid.NewGuid(),
            "Peak Season",
            Guid.NewGuid(),
            ...
            new List<UpdateRatePlanRoomCommand> { new(Guid.NewGuid(), 100m) });
    ...
    {
        // Arrange
        var command = CreateValidCommand() with { Rooms = new List<UpdateRatePlanRoomCommand>() };
    ...
    public void Validate_WithRoomWithEmptyRoomTypeId_ShouldHaveValidationError()
    {
        ...
        var command = CreateValidCommand() with
        {
            Rooms = new List<UpdateRatePlanRoomCommand> { new(Guid.Empty, 100m) }
        };
