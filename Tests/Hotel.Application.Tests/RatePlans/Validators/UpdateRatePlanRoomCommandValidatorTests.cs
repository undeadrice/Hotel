...
public class UpdateRatePlanRoomCommandValidatorTests
{
    private readonly UpdateRatePlanRoomCommandValidator _validator = new();

    [Fact]
    public void Validate_WithValidCommand_ShouldNotHaveValidationErrors()
    {
        // Arrange
        var command = new UpdateRatePlanRoomCommand(Guid.NewGuid(), 100m);
    ...
    {
        // Arrange
        var command = new UpdateRatePlanRoomCommand(Guid.Empty, 100m);
    ...
    {
        // Arrange
        var command = new UpdateRatePlanRoomCommand(Guid.NewGuid(), -1m);
